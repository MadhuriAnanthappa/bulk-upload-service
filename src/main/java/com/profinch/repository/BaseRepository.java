...
public abstract class BaseRepository<T> {
	...
	@Value("${app.procedure.cutoff-time}")
	private String cutoffTime;

	/**
	 * Performs batch insert using the provided SQL and setter for binding
	 * parameters.
	 */
	...
	protected void executeProcedure(String procedureCall, List<Object> args, boolean setNlsFormat) {
		...
		LocalTime cutoff = LocalTime.parse(cutoffTime);
		LocalTime now = LocalTime.now();

		if (now.isAfter(cutoff)) {
			log.error("[CUTOFF] Procedure execution blocked. Current time " + now + " is after cutoff time " + cutoff);
			return;
		}
	 ...
	 */
	protected Map<String, Object> fetchStatus(String summarySql, String detailSql, String reqRefNo, String detailKey) {
