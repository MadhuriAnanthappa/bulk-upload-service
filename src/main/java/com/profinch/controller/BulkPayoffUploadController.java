...
import com.profinch.dto.BulkPayoffUploadRequest;
import com.profinch.exception.TotalRecordsMismatchException;
import com.profinch.service.BulkPayoffUploadService;
import com.profinch.util.UploadResponseBuilder;

import lombok.extern.slf4j.Slf4j;
@Slf4j
@RestController
@RequestMapping("/api/bulkUploadPayoff")
public class BulkPayoffUploadController {
	@Autowired
	private BulkPayoffUploadService bulkPayoffUploadService;

	@PostMapping("/upload")
	public ResponseEntity<BulkApiResponse<Map<String, Object>>> bulkInsert(@Valid @RequestBody BulkPayoffUploadRequest request) {
		if (request.getTotalRecords() != request.getBulkPayoffUpload().size()) {
			throw new TotalRecordsMismatchException(request.getTotalRecords(),
					request.getBulkPayoffUpload().size());
		}

		String uploadStatus = (String) bulkPayoffUploadService.processBulkPayoffs(request.getBulkPayoffUpload(),
				request.getReqRefNo());
	...
	public ResponseEntity<BulkApiResponse<Map<String, Object>>> getStatus(@PathVariable String reqRefNo) {
		Map<String, Object> results = bulkPayoffUploadService.getStatusByReqRefNo(reqRefNo);
